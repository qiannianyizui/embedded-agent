# Path Resolution Overhaul Implementation Plan
...
- Test command: `./build/tests/ea-tests "[filesystem]"`
- Full test command: `./build/tests/ea-tests`
...
| `src/app/AppBuilder.cpp` | Ensure data_dir exists before creating databases |
...
**Interfaces:**
- Consumes: `home_dir()` (existing, returns `Result<std::string>`)
- Produces: `std::string expand_tilde(const std::string& path)` — expands `~` and `~/...` using `home_dir()`, returns non-tilde paths unchanged, returns tilde paths unchanged if `home_dir()` fails

- [ ] **Step 1: Write failing tests**

Add to `tests/unit/test_filesystem.cpp`:
...
TEST_CASE("expand_tilde does not expand ~otheruser", "[filesystem]") {
    // Only bare ~ and ~/... are expanded — ~username/ is left as-is
...
Run: `cmake --build build -j$(nproc) && ./build/tests/ea-tests "[filesystem]" 2>&1 | grep -E "expand_tilde|FAILED|no test"`
Expected: FAIL — `expand_tilde` not declared

- [ ] **Step 3: Implement `expand_tilde()`**

In `src/common/io/FileSystem.h`, add declaration after `data_dir()`:

```cpp
std::string expand_tilde(const std::string& path);
```

In `src/common/io/FileSystem.cpp`, add implementation before `home_dir()`:
...
**Interfaces:**
- Consumes: `home_dir()` (existing), `expand_tilde()` (from Task 1)
- Produces: Updated `config_dir()` — checks `EA_CONFIG_DIR` env var first, falls back to `home_dir()/.embedded-agent`
...
Run: `cmake --build build -j$(nproc) && ./build/tests/ea-tests "[filesystem]" 2>&1 | grep -E "EA_CONFIG_DIR|FAILED"`
Expected: FAIL — `config_dir()` doesn't check `EA_CONFIG_DIR` yet

- [ ] **Step 3: Implement `config_dir()` overhaul**

Replace `config_dir()` in `src/common/io/FileSystem.cpp`:
...
**Interfaces:**
- Consumes: `config_dir()` (from Task 2), `expand_tilde()` (from Task 1)
- Produces: Updated `data_dir()` — checks `EA_DATA_DIR` env var first, defaults to `config_dir()/data`
...
Run: `cmake --build build -j$(nproc) && ./build/tests/ea-tests "[filesystem]" 2>&1 | grep -E "EA_DATA_DIR|data_dir|FAILED"`
Expected: FAIL — `data_dir()` currently returns `config_dir()`, not `config_dir()/data`

- [ ] **Step 3: Implement `data_dir()` overhaul**

Replace `data_dir()` in `src/common/io/FileSystem.cpp`:
...
**Interfaces:**
- Consumes: `EA_CONFIG_DIR`, `EA_DATA_DIR` env vars, `config_dir()`, `expand_tilde()`
- Produces: Updated `data_dir()` — when `EA_CONFIG_DIR` is set, `EA_DATA_DIR` is ignored with a warning; data_dir becomes `config_dir()/data`
...
Run: `cmake --build build -j$(nproc) && ./build/tests/ea-tests "EA_CONFIG_DIR takes precedence" 2>&1 | grep -E "precedence|FAILED|PASSED"`
...
Run: `./build/tests/ea-tests "[filesystem]" 2>&1 | grep -E "precedence|PASSED|FAILED"`
Expected: PASS
...
**Interfaces:**
- Consumes: `data_dir()` (from Task 3/4)
- Produces: Simplified `resolve_data_path()` — directly uses `data_dir() + "/" + filename`

- [ ] **Step 1: Update existing tests to reflect new paths**

In `tests/unit/test_filesystem.cpp`, replace the existing `resolve_data_path returns config_dir/filename` test:
...
Run: `cmake --build build -j$(nproc) && ./build/tests/ea-tests "[filesystem]" 2>&1 | grep -E "resolve_data_path|FAILED"`
Expected: FAIL — path no longer contains `/data/` yet

- [ ] **Step 3: Simplify `resolve_data_path()`**

Replace `resolve_data_path()` in `src/common/io/FileSystem.cpp`:
...
**Interfaces:**
- Consumes: `ea::fs::config_dir()` (returns `Result<std::string>`)
...
Also add the include at the top if not already present:
```cpp
#include "common/io/FileSystem.h"
```

And remove the now-unused include if `ea::platform::home_dir()` is no longer used in main.cpp:
```cpp
// Remove: #include "platform/Platform.h"  (if no other platform:: symbols used)
```

Note: `ea::platform::platform_description()` is still used in main.cpp, so keep the Platform.h include.

- [ ] **Step 2: Update SetupWizardRunner.cpp log init**

In `src/app/SetupWizardRunner.cpp`, replace:
...
Also update the hermes detection line that uses `home`:

```cpp
    // Before: std::string hermes_path = home + "/.hermes";
    // After:
    auto home = ea::platform::home_dir();
    std::string hermes_path = home + "/.hermes";
```

Note: `home` is still needed for hermes detection, so keep the `platform::home_dir()` call for that purpose only.
...
**Interfaces:**
- Consumes: `ea::fs::data_dir()` (returns `Result<std::string>`), `ea::fs::mkdir_p()`

- [ ] **Step 1: Add data_dir mkdir_p in AppBuilder::build()**

In `src/app/AppBuilder.cpp`, after the lines:
